...
public class JwtTokenFilterConfigurerTest {
    ...
    @Test
    void configure_validProvider_filterAdded() {
        // given
        final JwtTokenFilterConfigurer sut = new JwtTokenFilterConfigurer(jwtTokenProvider);
        when(httpSecurity.addFilterBefore(any(Filter.class), eq(UsernamePasswordAuthenticationFilter.class))).thenReturn(httpSecurity);
