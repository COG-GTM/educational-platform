...
public class CourseByUUIDQueryHandlerTest {
    ...
    void handle_existingCourse_courseReturned() {
        ...
        final CourseDTO course = new CourseDTO(uuid, "name", "description", 5, new ArrayList<>());
        when(repository.findDTOByUuid(uuid)).thenReturn(Optional.of(course));
    ...
    @Test
    void handle_notExistingCourse_emptyReturned() {
        // given
        final UUID uuid = UUID.fromString("123e4567-e89b-12d3-a456-426655440001");
        final CourseByUUIDQuery query = new CourseByUUIDQuery(uuid);
