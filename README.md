...
Communication between bounded contexts is performed via integration events stored in a transactional outbox and dispatched to asynchronous listeners with Spring's `ApplicationEventPublisher`. The messaging infrastructure provides the following reliability guarantees:
...
- **Transactional outbox.** Command handlers publish events through `IntegrationEventOutbox`, which stores the serialized event in the `integration_event_outbox` table within the producing transaction (`@Transactional(propagation = MANDATORY)`), so an event becomes durable exactly when the business change commits and is discarded when it rolls back. A scheduled `IntegrationEventOutboxRelay` polls `NEW` entries, dispatches them via `ApplicationEventPublisher` inside its own transaction, and marks them `PROCESSED` — undelivered (`NEW`) events therefore survive process restarts and are dispatched at least once, though listener execution after dispatch remains at-most-once (see ADR-0015). Entries that can no longer be materialized are marked `FAILED` and logged.
- **After-commit publishing.** Listeners consume events with `@Async("integrationEventExecutor")` combined with `@TransactionalEventListener(phase = AFTER_COMMIT)`, so an event is only processed after the relay's dispatching transaction has committed successfully.
...
See [ADR-0014](docs/architecture-decisions/0014-async-integration-events-reliability.md) and [ADR-0015](docs/architecture-decisions/0015-transactional-outbox.md) for the rationale.
